 ...
 * 2. Sets tmux window name (fallback when Kitty unavailable)
 * 3. Persists per-window state for daemon recovery
 *
 * All hooks call setTabState() instead of directly running kitten/tmux commands.
 */
...
/**
 * Update tmux window name if running inside tmux.
 * Strips emoji prefixes and truncates for clean tmux status bar display.
 * On idle, re-enables automatic-rename so window reverts to process name.
 */
function setTmuxWindowName(title: string, idle: boolean = false): void {
  try {
    if (!process.env.TMUX) return;

    if (idle) {
      execSync('tmux set-window-option automatic-rename on', { stdio: 'ignore', timeout: 1000 });
      console.error('[tab-setter] tmux: re-enabled automatic-rename (idle)');
    } else {
      // Strip emoji prefixes and pipe-separated session labels for cleaner display
      const clean = title
        .replace(/^(?:🧠|⚙️|⚙|✓|❓|👁️|📋|🔨|⚡|✅|📚)\s*/, '')
        .replace(/^[A-Z\s]{2,}\|\s*/, '')  // Strip "SESSION NAME | " prefix
        .replace(/\.$/, '')
        .trim()
        .slice(0, 35);
      ...
      if (clean) {
        ...
        execSync(`tmux rename-window "${escaped}"`, { stdio: 'ignore', timeout: 1000 });
        console.error(`[tab-setter] tmux: renamed window to "${clean}"`);
      }
    }
  } catch { /* tmux not available or command failed — silent */ }
}
...
export function setTabState(opts: SetTabOptions): void {
  ...
  // Update tmux window name (works with or without Kitty)
  setTmuxWindowName(title, state === 'idle');

  // Kitty tab updates (requires Kitty terminal + socket)
  try {
...
export function setPhaseTab(phase: AlgorithmTabPhase, sessionId: string, summary?: string): void {
  ...
  // Update tmux window name (works with or without Kitty)
  setTmuxWindowName(title, phase === 'IDLE');
