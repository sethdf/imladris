#!/usr/bin/env bun
// $PAI_DIR/hooks/load-core-context.ts
// SessionStart hook: Inject USER identity context into Claude's context

import { existsSync, readFileSync, readdirSync } from 'fs';
import { join } from 'path';
import { homedir } from 'os';

interface SessionStartPayload {
  session_id: string;
  [key: string]: any;
}

function isSubagentSession(): boolean {
  // Check for subagent indicators
  // Subagents shouldn't load full context (they get it from parent)
  return process.env.CLAUDE_CODE_AGENT !== undefined ||
         process.env.SUBAGENT === 'true';
}

function getLocalTimestamp(): string {
  const date = new Date();
  const tz = process.env.TIME_ZONE || Intl.DateTimeFormat().resolvedOptions().timeZone;

  try {
    const localDate = new Date(date.toLocaleString('en-US', { timeZone: tz }));
    const year = localDate.getFullYear();
    const month = String(localDate.getMonth() + 1).padStart(2, '0');
    const day = String(localDate.getDate()).padStart(2, '0');
    const hours = String(localDate.getHours()).padStart(2, '0');
    const minutes = String(localDate.getMinutes()).padStart(2, '0');
    const seconds = String(localDate.getSeconds()).padStart(2, '0');

    return `${year}-${month}-${day} ${hours}:${minutes}:${seconds} PST`;
  } catch {
    return new Date().toISOString();
  }
}

// Key identity files to load (in order of priority)
const IDENTITY_FILES = [
  'DAIDENTITY.md',      // AI identity (Curu)
  'BASICINFO.md',       // User basic info (Seth)
  'TECHSTACKPREFERENCES.md', // Stack preferences
  'CONTACTS.md',        // Contact directory
  'Contacts.md',        // Alternative contacts file
  'CoreStack.md',       // Alternative stack file
];

async function main() {
  try {
    // Skip for subagents - they get context from parent
    if (isSubagentSession()) {
      process.exit(0);
    }

    const stdinData = await Bun.stdin.text();
    if (!stdinData.trim()) {
      process.exit(0);
    }

    const payload: SessionStartPayload = JSON.parse(stdinData);
    const paiDir = process.env.PAI_DIR || join(homedir(), '.claude');
    const userDir = join(paiDir, 'USER');

    if (!existsSync(userDir)) {
      // No USER directory - skip
      console.error('[PAI] No USER directory found - skipping context injection');
      process.exit(0);
    }

    // Collect content from identity files
    const contextParts: string[] = [];

    for (const filename of IDENTITY_FILES) {
      const filePath = join(userDir, filename);
      if (existsSync(filePath)) {
        const content = readFileSync(filePath, 'utf-8');
        contextParts.push(`## ${filename}\n\n${content}`);
      }
    }

    if (contextParts.length === 0) {
      console.error('[PAI] No identity files found in USER/ - skipping context injection');
      process.exit(0);
    }

    // Build context summary
    const identityContext = contextParts.join('\n\n---\n\n');

    // Output as system-reminder for Claude to process
    const output = `<system-reminder>
PAI CORE CONTEXT (Auto-loaded at Session Start)

📅 CURRENT DATE/TIME: ${getLocalTimestamp()}

The following identity context has been loaded from ${userDir}:

${identityContext}

This context is now active for this session. Follow all instructions, preferences, and guidelines contained above.
</system-reminder>

✅ PAI Context successfully loaded...`;

    // Output goes to stdout - Claude Code will see it
    console.log(output);

  } catch (error) {
    // Never crash - just skip
    console.error('Context loading error:', error);
  }

  process.exit(0);
}

main();
