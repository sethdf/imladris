...
/**
 * Load the most recent compaction checkpoint summary.
 * Only surfaces it if it's recent (within 7 days) and non-trivial.
 */
function loadLastCompactionCheckpoint(paiDir: string): string | null {
  const path = join(paiDir, 'MEMORY', 'STATE', 'last-compaction.json');
  if (!existsSync(path)) return null;
  try {
    const data: LastCompaction = JSON.parse(readFileSync(path, 'utf-8'));
    if (!data.summary_preview || data.summary_preview.length < 20) return null;
    const age = Date.now() - new Date(data.timestamp).getTime();
    const sevenDays = 7 * 24 * 60 * 60 * 1000;
    if (age > sevenDays) return null;
    const ageHours = Math.round(age / (1000 * 60 * 60));
    const ageLabel = ageHours < 1 ? 'just now' : ageHours < 24 ? `${ageHours}h ago` : `${Math.round(ageHours / 24)}d ago`;
    return `\n  ── Last Compaction Checkpoint (${ageLabel}) ──\n` +
      `  📌 ${data.task_title}\n` +
      `  ${data.summary_preview.slice(0, 300).replace(/\n/g, '\n  ')}\n`;
  } catch {
    return null;
  }
}
...
async function checkActiveProgress(paiDir: string): Promise<string | null> {
  ...
  const compactionCheckpoint = loadLastCompactionCheckpoint(paiDir);

  if (recentSessions.length === 0 && projects.length === 0 && !compactionCheckpoint) {
    return null;
  }
