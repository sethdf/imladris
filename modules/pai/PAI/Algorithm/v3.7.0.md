...
| Ruflo Swarm | Code-specific multi-file work (>=2 files). Delegates ISC criteria to Ruflo agent swarm via bridge. Additive-only — never replaces PAI capabilities. | `Bash("bun run ~/repos/imladris/scripts/pai-ruflo-bridge.ts --prd <prd-path>")` |
