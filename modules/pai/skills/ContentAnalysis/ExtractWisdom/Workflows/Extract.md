...
**Save extraction to disk before returning** — wisdom from content is one-time-per-source.

1. Generate a `SLUG` from the content title/source: lowercase, hyphens, ≤30 chars
2. Use the **Write tool** to create:
   `~/.claude/History/research/YYYY-MM/YYYY-MM-DD_[SLUG]/output.md`
   with the full wisdom extraction
3. If a work item is active, also copy there:
   ...
   # cp History output → ~/.claude/MEMORY/WORK/$WORK_DIR/wisdom-$(date +%H%M%S).md
   ```
