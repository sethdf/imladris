...
# =============================================================================
# BWS (Bitwarden Secrets) is installed on the EC2 workstation (imladris), not
# on Aurora. This script SSHs to EC2 to retrieve the secret and writes it to
# the local PAI env file.
#
# Usage:
#   ./sync-aurora-credentials.sh              # Default: SSH to imladris
#   SSH_HOST=imladris ./sync-aurora-credentials.sh   # Custom host
#
# Prerequisites:
#   - SSH access to EC2 via Tailscale (imladris)
#   - BWS configured on EC2 with access token
#   - jq installed locally

set -euo pipefail

SSH_HOST="${SSH_HOST:-imladris}"
