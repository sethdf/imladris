...
import { shouldCatchUp, recordRun, type CatchupInfo } from "../../core/catchup_lib.ts";
...
export async function main(
  ...
  try {
    const { store, isAvailable, init } = await import("../../core/cache_lib.ts");
    if (isAvailable()) {
      init();
  ...
  try {
    const cacheLib = await import("../../core/cache_lib.ts");
